...
namespace Dynamixel {
  ...
  public class Servo_xl430_w250_t {
    ...
    private byte[] CreateDynamixelCommandUInt32(byte id, byte instruction, int address, int value) {
      ...
      txbuffer.AddRange(BitConverter.GetBytes((Int32)value));
    ...
    private byte[] CreateDynamixelCommand(byte id, byte instruction, int address, int value) {
      ...
      txbuffer.AddRange(BitConverter.GetBytes((UInt16)value));
    ...
    public byte[] TorqueEnable(byte id, bool status) {
      ...
      return CreateDynamixelCommandByte(id, 3, 64, (byte)(status ? 1 : 0));
    }
    ...
    public byte[] LED(byte id, bool status) {
      ...
      return CreateDynamixelCommandByte(id, 3, 65, (byte)(status ? 1 : 0));
    }

    public byte[] DisableStatusPacket(byte id) {

      return CreateDynamixelCommandByte(id, 0x03, 68, 1);
    }
    ...
    public byte[] ChangeID(byte fromId, byte toId) {
      ...
      return CreateDynamixelCommandByte(fromId, 3, 7, toId);
    }

    public byte[] SendPing(byte id) {

      return CreateDynamixelCommand(id, 1);
    }
